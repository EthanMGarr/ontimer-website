...
Location controls follow one shared mobile-first rule: “Use my current location” is shown only until geolocation succeeds. After success, the location field itself displays “Current location”; the trigger and visible confirmation message disappear, while an assistive-technology status remains. Editing the field restores the trigger. Route-swap controls may be visually compact, but must retain a minimum 44 by 44 pixel touch target. Calculator result content and handoff links must wrap within the viewport, and both `html` and `body` retain `overflow-x: clip` as a final containment guard rather than a substitute for fixing an overflowing child. Mobile text-entry controls must render at 16px or larger so iOS Safari does not zoom the visual viewport on focus and leave the page horizontally pannable; never prevent user zoom as a workaround.
