...
- For recurring countdown pages, use the natural `{event} countdown` phrase consistently in the title, description, H1, structured-data name, breadcrumb, and internal-link anchor. Keep the numeric days-until answer and full target date in the description; do not trade the direct answer for generic excitement copy.
...
- Hydrate recurring date answers from the same server-stable target instant and date label before switching to the visitor's local clock; a UTC build and local browser must not produce a React text mismatch.
